...
## Facade - 단순한 창구
Facade 패턴
- 커다란 프로그램을 사용해 처리를 실행하려면 상호 관련된 많은 클래스를 적절하게 제어해야 한다. 
- 그 처리를 실행하기 위한 '창구'를 준비해 두는게 좋다. 
- 많은 클래스를 개별적으로 제어하지 않아도 그 '창구'에 대해서만 요구하면 역할은 끝난다. 

Facade 패턴의 등장인물
- Facade의 역할
  - 시스템을 구성하고 있는 그밖의 많은 역할에 대해 '단순한 창구'가 도니다. 
...
- 어떤 요청이 발생했을 때 그 요청을 처리할 오브젝트를 직접 결정할 수 없는 경우, 복수의 오브젝트를 사슬처럼 연결해 두면, 그 오브젝트의 사슬을 차례대로 돌아다니면서 목적한 오브젝트를 결정하는 방법이다.
- 요청하는 쪽과 처리하는 쪽의 연결을 유연하게 해각 오브젝트를 부품으로 독립시킬 수 있다. 

Chain of Responsibility 패턴의 등장인물
- Handler의 역할
  - 요구사항을 처리하는 인터페이스를 결정한다. 
  - 다음 사람을 준비해 두고 자신이 처리할 수 없는 요구가 나오면 그 다음사람에게 떠넘긴다.
  - 예제의 Support
- ConcreteHandler의 역할
